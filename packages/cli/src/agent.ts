...
export function ensureDocumentContextSection(markdown: string, rawPrompt: unknown) {
  ...
  let section = '## Document context\n\n> Work notes and background from selected documents. Document-reported work is attributed to these sources; plans and open actions do not imply completion.\n';
...
export function contextPrompt(context: any, clones: Config['clones']) {
  ...
  if (workspaceReport) text += `This report covers the whole workspace. Contributors with qualifying evidence: ${contributors.join(', ') || 'none'}. Include every listed contributor and attribute their work accurately. Also include contributors whose completed work is explicitly described in the supplied document context, even if they have no Git events; do not infer ownership merely from a list of people. Organize the result with a workspace overview followed by a clearly labeled section for each contributor; do not collapse the report into the job owner's contributions. Use each engineer's name alone as a level-two heading (for example, "## Ibrahim"), without bold or inline code. Use level-three headings for any project subsections so engineer names remain the primary visual landmarks.\n\n`;
  text += format === 'summary'
