...
import {registerWatchedRoots, syncHistory} from '../packages/cli/src/agent.js';
...
describe('repository history refresh', () => {
  ...
  it('imports 90 days first, then imports newly reachable commits even when their timestamp is older', async () => {
    ...
    const secondInitialSha = commitAt(repo, 'second commit inside initial window', new Date(Date.now() - 5 * 24 * 60 * 60_000));
    ...
    expect(await registerWatchedRoots(config)).toBe(1);
    expect(activities).toEqual([]);
    expect(await syncHistory(config)).toEqual({repositories: 1, commits: 2});
    expect(activities.map(event => event.data.commitSha)).toEqual([initialSha, secondInitialSha]);
    expect(maximumActiveUploads).toBe(2);
    const firstHeads = config.clones[0].historyHeads;
    expect(firstHeads).toContain(secondInitialSha);
    ...
    expect(await syncHistory(config)).toEqual({repositories: 1, commits: 1});
