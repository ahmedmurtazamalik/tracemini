...
describe('PostgreSQL database', () => {
  ...
  it('opens a migrated Postgres database with the TraceMini schema', async () => {
    ...
    try {
      ...
      expect(migrations.rows.map((row: any) => row.version)).toEqual([1, 3, 4, 5, 6, 9, 11, 12, 13, 14, 15, 16, 17, 18, 19]);
      ...
      expect(reportJobColumns.rows.map((row: any) => row.column_name)).toEqual(expect.arrayContaining(['custom_prompt', 'target_report_id', 'report_name', 'format', 'report_scope', 'schedule_id', 'scheduled_for', 'coalesced_runs']));
