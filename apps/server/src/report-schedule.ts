...
export async function materializeDueReportSchedules(db: DB, userId: number, at = new Date()) {
  ...
  for (const item of due) {
    created += await db.transaction(async () => {
      ...
      const result = await db.prepare("INSERT INTO report_jobs(workspace_id,user_id,reporter,start_date,end_date,timezone,include_diff,status,report_name,format,report_scope,schedule_id,scheduled_for,coalesced_runs,created_at) VALUES(?,?,?,?,?,?,?,'pending',? ,?,'workspace',?,?,?,?) ON CONFLICT (schedule_id,scheduled_for) DO NOTHING RETURNING id")
        .run(schedule.workspace_id, schedule.configured_by, schedule.reporter, startDate, endDate, schedule.timezone, schedule.include_diff, `Scheduled workspace report · ${startDate} — ${endDate}`, schedule.format, schedule.id, scheduledFor.toISOString(), coalescedRuns, at.toISOString());
