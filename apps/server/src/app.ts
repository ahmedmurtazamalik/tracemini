...
export function createApp(db: DB, webDir?: string, cliDir = defaultCliDir) {
  ...
  const userAuth = async (req: Authed, res: Response, next: NextFunction) => {
    ...
    const row = raw && await db.prepare('SELECT u.* FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.token_hash=?').get(hash(raw));
  ...
  app.post('/api/auth/register', required(['name', 'email', 'password']), async (req, res, next) => {
    try {
      ...
      const userId = await db.transaction(async () => {
        const result = await db.prepare('INSERT INTO users(name,email,password_hash,created_at) VALUES(?,?,?,?) RETURNING id').run(req.body.name.trim(), req.body.email.trim().toLowerCase(), bcrypt.hashSync(req.body.password, 10), now());
        await db.prepare('INSERT INTO sessions VALUES(?,?,?)').run(hash(raw), result.lastInsertRowid, now());
        return Number(result.lastInsertRowid);
      });
      res.status(201).json({token: raw, user: {id: userId, name: req.body.name, email: req.body.email.toLowerCase()}});
    } catch (error: any) {
  ...
  app.post('/api/auth/login', required(['email', 'password']), async (req, res) => {
    ...
    const raw = token();
    await db.prepare('INSERT INTO sessions VALUES(?,?,?)').run(hash(raw), user.id, now());
  ...
  app.get('/api/workspaces/:id/reports', userAuth, requireMember, async (req, res) => res.json(await db.prepare('SELECT r.*,u.name user_name FROM reports r JOIN users u ON u.id=r.user_id WHERE r.workspace_id=? ORDER BY r.created_at DESC').all(req.params.id)));
  app.get('/api/reports/:id', userAuth, async (req: Authed, res) => { const row = await db.prepare('SELECT r.* FROM reports r JOIN workspace_members wm ON wm.workspace_id=r.workspace_id WHERE r.id=? AND wm.user_id=?').get(req.params.id, req.user.id); row ? res.json(row) : res.status(404).json({error: 'not found'}); });
