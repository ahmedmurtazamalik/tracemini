...
export const HELP_SECTIONS: HelpSection[] = [
  ...
  {number: '02', title: 'Install the local CLI', description: 'TraceMini needs its Linux CLI on each developer machine that will contribute Git activity or generate reports.', detail: 'Open Install CLI, create the one-time command, and run it in a terminal. The CLI sends activity metadata only—your source code remains on your device.', destination: 'install', action: 'Open Install CLI'},
  {number: '03', title: 'Approve folders and find repositories', description: 'Choose exactly which parent folders TraceMini is allowed to scan on each device.', detail: 'Run “tracemini watch /absolute/path” in a terminal. The absolute path can contain one repository or many nested repositories. Run the command again for additional roots. Saved roots are device-wide and available when you scan any workspace; TraceMini never scans outside them or uploads source code.', destination: 'settings', action: 'Open repository settings'},
  {number: '04', title: 'Select repositories to trace', description: 'Scanning all approved roots finds candidates; tracing each repository is a separate, workspace-specific choice.', detail: 'Turn on the switch beside repositories from your own devices. The agent validates identity and installs managed Git hooks. You can remove an untraced result without deleting files or history, but a later scan can rediscover it while it remains under an approved root.', destination: 'settings', action: 'Choose repositories'},
