      ...
      <section className="card reports-create-card document-context-card">
        ...
        <div className="actions">
          <label className={`button secondary file-button${documentPending || documents.length >= 5 ? " disabled" : ""}`}>
            {documentPending ? "Analyzing locally…" : "Add context files"}
            <input type="file" multiple accept=".pdf,.pptx,.md,.txt,application/pdf,application/vnd.openxmlformats-officedocument.presentationml.presentation,text/markdown,text/plain,text/x-markdown" disabled={documentPending || documents.length >= 5} onChange={async event => {
