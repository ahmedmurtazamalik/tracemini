...
export function ReportSchedule({api, workspaceId, timezone}: {api: Api; workspaceId: number; timezone: string}) {
  ...
  const save = async (event: FormEvent) => {
    ...
    try {
      ...
      if (!alive.current || generation !== loadGeneration.current) return; setName(schedule.name); setNextRun(schedule.next_run_at); setMessage(enabled ? 'Schedule saved.' : 'Schedule paused.');
    } catch (caught: any) { if (alive.current && generation === loadGeneration.current) setError(caught.message); }
    finally { if (alive.current && generation === loadGeneration.current) setPending(false); }
  };
  return <section className="card reports-create-card schedule-card">
    <div className="section-heading"><div><span>Automatic reports</span><h2>Schedule workspace reports</h2></div></div>
    ...
    {loading ? <div className="alert progress" role="status">Loading schedule…</div> : <form className="reports-controls" onSubmit={save}>
